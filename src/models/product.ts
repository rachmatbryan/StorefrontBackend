    ...
    async index(): Promise<Product[]> {
      ...
      async create(product:Product): Promise<Product> {
        try {
            ...
            throw new Error(`Could not add item ${product.product_name}. Error: ${err}`)
        }
      }
      ...
      async delete(product_id: number): Promise<Product> {
        try {
            ...
            throw new Error(`Product not found or already deleted with ID: ${product_id}`);
        } catch (err) {
            throw new Error(`Could not delete product with ID ${product_id}. Error: ${err}`);
        }
    }
}
